...
export default function MapPlaceholder() {
  ...
  const handleCalculatePath = () => {
    ...
    const obstacleRadius = 2; // Radius in grid units to block around an obstacle

    // Add tactical icons (except start/end) and decoys as obstacles
    const allObstacles = [
        ...
        for(let x = -obstacleRadius; x <= obstacleRadius; x++) {
            for(let y = -obstacleRadius; y <= obstacleRadius; y++) {
                ...
                if (checkX >= 0 && checkX < GRID_SIZE && checkY >= 0 && checkY < GRID_SIZE) {
                    if (x*x + y*y <= obstacleRadius*obstacleRadius) { // circular obstacle area
